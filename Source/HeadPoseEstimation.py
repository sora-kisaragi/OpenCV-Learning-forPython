...
# 顔から目や鼻などのランドマークを出力する
predictor = dlib.shape_predictor(predictor_path)

# カメラから画像を取得し続ける
while True:
    # カメラからキャプチャしてframeに1コマ分の画像データを入れる
    ...
    if len(rects) > 0:
        ...
        cv2.putText(frame, 'pitch : ' + str(int(pitch)), (20, 25),
                    cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 2)
        cv2.putText(frame, 'roll : ' + str(int(roll)), (20, 40),
                    cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 2)
        ...
        for p in image_points:
            cv2.drawMarker(frame, (int(p[0]), int(
                p[1])),  (0.0, 1.409845, 255), markerType=cv2.MARKER_CROSS, thickness=1)

        p1 = (int(image_points[0][0]), int(image_points[0][1]))
        p2 = (int(nose_end_point2D[0][0][0]), int(nose_end_point2D[0][0][1]))

        cv2.arrowedLine(frame, p1, p2, (255, 20, 0), 2)
